...
function ProspectContext({ persona }: { persona: Persona }) {
  ...
  const objections = ['Price is too high vs. current distributor', 'Switching costs and operational disruption', 'Unproven velocity in this territory', 'Already over-extended on shelf/tap space'];
  const winConditions = ['Acknowledge the switching risk and offer a trial period', 'Bring data: velocity, margin lift, brand awareness', 'Differentiate on service, not just product', 'Ask for a small commitment, not the whole relationship'];
  return (
    <Card>
...
function AnalysisView({ persona, onReplay, onPractice, onBack }: { persona: Persona; onReplay: () => void; onPractice: () => void; onBack: () => void }) {
...
function LiveCallPanel({ messages, status, interimTranscript, callTimer, isMuted, isPaused, onMicClick, onEnd, onMute, onPause, persona }: {
